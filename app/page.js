...
const slides = [
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "52px 60px 36px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, flex: 1 }}>
          {[
            ...
            { label: "Contact Pool", value: "~8,705 total (7,705 purchased practitioner list + ~1,000 conference attendees)" },
  ...
  // 3.5: CONTENT CREATION
  {
    content: (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "52px 60px 36px" }}>
        <div style={{ fontSize: 12, letterSpacing: 2, color: C.green, textTransform: "uppercase", marginBottom: 8 }}>Authority Engine</div>
        <h2 style={{ fontSize: 32, fontWeight: 700, color: C.text, marginBottom: 28 }}>Content Creation Process</h2>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 20 }}>
          <div style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24 }}>
            <div style={{ fontSize: 13, color: C.green, textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 600, marginBottom: 16 }}>How It Works</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                { step: "1", label: "Record", desc: "1–2 hour session with Alka. Conversational format covering product expertise, practitioner insights, and industry perspective." },
                { step: "2", label: "Extract", desc: "AI-assisted content extraction from the session transcript. Each session produces multiple content types from a single recording." },
                { step: "3", label: "Review", desc: "Content batch delivered to Alka for review and approval within 1–2 weeks of the session. Nothing goes external without sign-off." },
              ].map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: C.greenDim, border: `1px solid ${C.green}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: C.green }}>{s.step}</span>
                  </div>
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 600, color: C.text, marginBottom: 4 }}>{s.label}</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24, flex: 1 }}>
              <div style={{ fontSize: 13, color: C.accent, textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 600, marginBottom: 16 }}>What Each Session Produces</div>
              {[
                { type: "Articles", desc: "1–2 long-form pieces (600–900 words) targeting specific practitioner segments", color: C.accent },
                { type: "Social Posts", desc: "4–6 posts formatted for LinkedIn and Instagram, ready to publish", color: C.blue },
                { type: "Email Inserts", desc: "Social proof blocks, credibility statements, and objection responses that feed directly into the outreach sequences", color: "#ec4899" },
                { type: "Conference Collateral", desc: "Talking points, panel prep, elevator pitch, networking soundbites (Session 1 only — before April conference)", color: "#f97316" },
              ].map((t, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: i < 3 ? 12 : 0 }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: t.color, marginTop: 7, flexShrink: 0 }} />
                  <div>
                    <span style={{ fontSize: 15, fontWeight: 600, color: C.text }}>{t.type}</span>
                    <span style={{ fontSize: 14, color: C.muted }}> — {t.desc}</span>
                  </div>
                </div>
              ))}
            </div>
            <div style={{ backgroundColor: C.greenDim, border: `1px solid ${C.green}30`, borderRadius: 14, padding: 20 }}>
              <div style={{ fontSize: 15, color: C.text, lineHeight: 1.6 }}>
                <span style={{ fontWeight: 600, color: C.green }}>Delivery:</span> 1–2 weeks from session date. All content delivered as a batch for Alka's review and approval before use.
              </div>
            </div>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
          {[
            { session: "Session 1", date: "Mar 5", status: "Recorded. Content in production.", color: C.green },
            { session: "Session 2", date: "April (TBD)", status: "To be scheduled with Alka.", color: C.muted },
            { session: "Session 3", date: "May (TBD)", status: "To be scheduled with Alka.", color: C.muted },
          ].map((s, i) => (
            <div key={i} style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <span style={{ fontSize: 15, fontWeight: 600, color: C.text }}>{s.session}</span>
                <span style={{ fontSize: 13, color: s.color }}>{s.date}</span>
              </div>
              <div style={{ fontSize: 13, color: C.muted }}>{s.status}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    content: (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "52px 60px 36px" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            { item: "Purchased practitioner list (7,705 contacts)", when: "Before Mar 17 launch", status: "Received", note: "List received. Classification and verification in progress", color: C.green },
